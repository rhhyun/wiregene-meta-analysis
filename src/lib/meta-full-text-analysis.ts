...
const openAiFullTextKeywordPattern =
  /(?:abstract|method|methods|participants|respondents|sample|survey|questionnaire|result|results|prevalence|pain|musculoskeletal|PRMD|injury|instrument|guitar|classical\s+guitar|bassoon|bassoonist|violin|viola|cello|piano|orchestra|table|figure|appendix|thenar|posterior\s+neck|lower\s+back|occupational\s+health|neck|cervical|shoulder|interscapular|shoulder\s+blades|head\s+or\s+neck|back.*chest.*shoulder|arms?\s+or\s+wrists?|legs?\s+or\s+hips?|hands|neutral\s+arm|elevated\s+arm|work\s+postures|posture|playing\s+time|VAS|NDI|SF-36|quality\s+of\s+life|disability|symptomatic|attrition|student|students|sex|gender|male|female|missing|location|diagnosed|Yates|chi[-\s]?square|cohort|overlap|discussion|conclusion)/gi;
